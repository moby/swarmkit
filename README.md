...
- **Distributed**: *SwarmKit* uses the [Raft Consensus Algorithm](https://raft.github.io/) in order to maintain consistent state and coordinate without a single point of failure to make decisions.
...
* Dynamic cluster role promotion and demotion
* Desired/actual state reconciliation
* Node failover
* Node maintenance mode for zero-downtime system upgrades
* Service scaling
* Lock-step and rolling updates (for canary, blue/green, other application deployments)
...
In two additional terminals, join two nodes (note: replace `127.0.0.1:2377` with the address of the first node)

```sh
$ swarmd -d /tmp/node-2 --hostname node-2 --join-addr 127.0.0.1:2377
$ swarmd -d /tmp/node-3 --hostname node-3 --join-addr 127.0.0.1:2377
```
