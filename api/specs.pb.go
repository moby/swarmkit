...
type SecretSpec struct {
	...
	// Types that are valid to be assigned to SpecialSecretType:
	//	*SecretSpec_CA
	//	*SecretSpec_LeafCertificate
	SpecialSecretType isSecretSpec_SpecialSecretType `protobuf_oneof:"special_secret_type"`
}
...
type isSecretSpec_SpecialSecretType interface {
	isSecretSpec_SpecialSecretType()
	MarshalTo([]byte) (int, error)
	Size() int
}

type SecretSpec_CA struct {
	CA *CAInfo `protobuf:"bytes,5,opt,name=ca,oneof"`
}
type SecretSpec_LeafCertificate struct {
	LeafCertificate *LeafCertInfo `protobuf:"bytes,6,opt,name=leaf_certificate,json=leafCertificate,oneof"`
}

func (*SecretSpec_CA) isSecretSpec_SpecialSecretType()              {}
func (*SecretSpec_LeafCertificate) isSecretSpec_SpecialSecretType() {}

func (m *SecretSpec) GetSpecialSecretType() isSecretSpec_SpecialSecretType {
	if m != nil {
		return m.SpecialSecretType
	}
	return nil
}

func (m *SecretSpec) GetCA() *CAInfo {
	if x, ok := m.GetSpecialSecretType().(*SecretSpec_CA); ok {
		return x.CA
	}
	return nil
}

func (m *SecretSpec) GetLeafCertificate() *LeafCertInfo {
	if x, ok := m.GetSpecialSecretType().(*SecretSpec_LeafCertificate); ok {
...
// XXX_OneofFuncs is for the internal use of the proto package.
func (*SecretSpec) XXX_OneofFuncs() (func(msg proto.Message, b *proto.Buffer) error, func(msg proto.Message, tag, wire int, b *proto.Buffer) (bool, error), func(msg proto.Message) (n int), []interface{}) {
	return _SecretSpec_OneofMarshaler, _SecretSpec_OneofUnmarshaler, _SecretSpec_OneofSizer, []interface{}{
		(*SecretSpec_CA)(nil),
		(*SecretSpec_LeafCertificate)(nil),
	}
}

func _SecretSpec_OneofMarshaler(msg proto.Message, b *proto.Buffer) error {
	m := msg.(*SecretSpec)
	// special_secret_type
	switch x := m.SpecialSecretType.(type) {
	case *SecretSpec_CA:
		_ = b.EncodeVarint(5<<3 | proto.WireBytes)
		if err := b.EncodeMessage(x.CA); err != nil {
			return err
		}
	case *SecretSpec_LeafCertificate:
		_ = b.EncodeVarint(6<<3 | proto.WireBytes)
		if err := b.EncodeMessage(x.LeafCertificate); err != nil {
			return err
		}
	case nil:
	default:
		return fmt.Errorf("SecretSpec.SpecialSecretType has unexpected type %T", x)
	}
	return nil
}

func _SecretSpec_OneofUnmarshaler(msg proto.Message, tag, wire int, b *proto.Buffer) (bool, error) {
	m := msg.(*SecretSpec)
	switch tag {
	case 5: // special_secret_type.ca
		if wire != proto.WireBytes {
			return true, proto.ErrInternalBadWireType
		}
		msg := new(CAInfo)
		err := b.DecodeMessage(msg)
		m.SpecialSecretType = &SecretSpec_CA{msg}
		return true, err
	case 6: // special_secret_type.leaf_certificate
		if wire != proto.WireBytes {
			return true, proto.ErrInternalBadWireType
		}
		msg := new(LeafCertInfo)
		err := b.DecodeMessage(msg)
		m.SpecialSecretType = &SecretSpec_LeafCertificate{msg}
		return true, err
	default:
		return false, nil
	}
...
func _SecretSpec_OneofSizer(msg proto.Message) (n int) {
	m := msg.(*SecretSpec)
	// special_secret_type
	switch x := m.SpecialSecretType.(type) {
	case *SecretSpec_CA:
		...
		n += s
	case *SecretSpec_LeafCertificate:
		s := proto.Size(x.LeafCertificate)
...
// CAInfo is the certificate info part of a CA.  We are not using CAConfig because the signing key
// is part of the secret data, and CAInfo is intended to be extra information attached to a Secret object.
type CAInfo struct {
	...
	// Certificate is the certificate that matches the private key stored in the Data field in a CA type secret
	Certificate []byte `protobuf:"bytes,3,opt,name=certificate,proto3" json:"certificate,omitempty"`
	// CertBundle are optional extra root certificates to include with the CA certificate
	CertBundle []byte `protobuf:"bytes,4,opt,name=cert_bundle,json=certBundle,proto3" json:"cert_bundle,omitempty"`
}

func (m *CAInfo) Reset()                    { *m = CAInfo{} }
func (*CAInfo) ProtoMessage()               {}
func (*CAInfo) Descriptor() ([]byte, []int) { return fileDescriptorSpecs, []int{14} }

// LeafCertInfo is the certificate part of a TLS key-pair secret.  It is intended to be extra information
// that is attached to a Secret object.
type LeafCertInfo struct {
	// Certificate is the certificate that matches the private key stored in the Data field in a LeafCert type secret
	Certificate []byte `protobuf:"bytes,1,opt,name=certificate,proto3" json:"certificate,omitempty"`
}

func (m *LeafCertInfo) Reset()                    { *m = LeafCertInfo{} }
func (*LeafCertInfo) ProtoMessage()               {}
func (*LeafCertInfo) Descriptor() ([]byte, []int) { return fileDescriptorSpecs, []int{15} }
...
func init() {
	...
	proto.RegisterType((*CAInfo)(nil), "docker.swarmkit.v1.CAInfo")
	proto.RegisterType((*LeafCertInfo)(nil), "docker.swarmkit.v1.LeafCertInfo")
...
func (m *SecretSpec) CopyFrom(src interface{}) {
	...
	*m = *o
	...
	if o.SpecialSecretType != nil {
		switch o.SpecialSecretType.(type) {
		case *SecretSpec_CA:
			v := SecretSpec_CA{
				CA: &CAInfo{},
			}
			github_com_docker_swarmkit_api_deepcopy.Copy(v.CA, o.GetCA())
			m.SpecialSecretType = &v
		case *SecretSpec_LeafCertificate:
			v := SecretSpec_LeafCertificate{
				LeafCertificate: &LeafCertInfo{},
			}
			github_com_docker_swarmkit_api_deepcopy.Copy(v.LeafCertificate, o.GetLeafCertificate())
			m.SpecialSecretType = &v
		}
	}
...
func (m *CAInfo) CopyFrom(src interface{}) {
	...
	*m = *o
	...
	if o.CertBundle != nil {
		m.CertBundle = make([]byte, len(o.CertBundle))
		copy(m.CertBundle, o.CertBundle)
	}
}

func (m *LeafCertInfo) Copy() *LeafCertInfo {
	if m == nil {
		return nil
	}
	o := &LeafCertInfo{}
...
func (m *LeafCertInfo) CopyFrom(src interface{}) {
...
func (m *SecretSpec) MarshalTo(dAtA []byte) (int, error) {
	...
	if m.SpecialSecretType != nil {
		nn39, err := m.SpecialSecretType.MarshalTo(dAtA[i:])
		if err != nil {
			return 0, err
		}
		i += nn39
...
func (m *SecretSpec_CA) MarshalTo(dAtA []byte) (int, error) {
	i := 0
	if m.CA != nil {
		dAtA[i] = 0x2a
		i++
		i = encodeVarintSpecs(dAtA, i, uint64(m.CA.Size()))
		n40, err := m.CA.MarshalTo(dAtA[i:])
		if err != nil {
			return 0, err
		}
		i += n40
...
func (m *SecretSpec_LeafCertificate) MarshalTo(dAtA []byte) (int, error) {
	i := 0
	if m.LeafCertificate != nil {
		dAtA[i] = 0x32
		i++
		i = encodeVarintSpecs(dAtA, i, uint64(m.LeafCertificate.Size()))
		n41, err := m.LeafCertificate.MarshalTo(dAtA[i:])
		if err != nil {
			return 0, err
		}
		i += n41
...
func (m *CAInfo) Marshal() (dAtA []byte, err error) {
	size := m.Size()
	dAtA = make([]byte, size)
	n, err := m.MarshalTo(dAtA)
	if err != nil {
...
func (m *CAInfo) MarshalTo(dAtA []byte) (int, error) {
	var i int
	_ = i
	var l int
	_ = l
	if m.NodeCertExpiry != nil {
		dAtA[i] = 0xa
		i++
		i = encodeVarintSpecs(dAtA, i, uint64(m.NodeCertExpiry.Size()))
		n44, err := m.NodeCertExpiry.MarshalTo(dAtA[i:])
		if err != nil {
			return 0, err
		}
		i += n44
	...
	if len(m.Certificate) > 0 {
		dAtA[i] = 0x1a
		i++
		i = encodeVarintSpecs(dAtA, i, uint64(len(m.Certificate)))
		i += copy(dAtA[i:], m.Certificate)
	}
	if len(m.CertBundle) > 0 {
		dAtA[i] = 0x22
		i++
		i = encodeVarintSpecs(dAtA, i, uint64(len(m.CertBundle)))
		i += copy(dAtA[i:], m.CertBundle)
	}
	return i, nil
}

func (m *LeafCertInfo) Marshal() (dAtA []byte, err error) {
	size := m.Size()
	dAtA = make([]byte, size)
	n, err := m.MarshalTo(dAtA)
	if err != nil {
...
func (m *LeafCertInfo) MarshalTo(dAtA []byte) (int, error) {
	var i int
	_ = i
	var l int
	_ = l
...
func (m *SecretSpec) Size() (n int) {
	...
	if m.SpecialSecretType != nil {
		n += m.SpecialSecretType.Size()
	}
	return n
}

func (m *SecretSpec_CA) Size() (n int) {
	var l int
	_ = l
	if m.CA != nil {
		l = m.CA.Size()
...
func (m *SecretSpec_LeafCertificate) Size() (n int) {
	var l int
	_ = l
	if m.LeafCertificate != nil {
		l = m.LeafCertificate.Size()
		n += 1 + l + sovSpecs(uint64(l))
	}
	return n
}
...
func (m *LeafCertInfo) Size() (n int) {
	var l int
	_ = l
	l = len(m.Certificate)
	if l > 0 {
...
func (this *SecretSpec) String() string {
	...
	s := strings.Join([]string{`&SecretSpec{`,
		...
		`SpecialSecretType:` + fmt.Sprintf("%v", this.SpecialSecretType) + `,`,
		`}`,
	}, "")
	return s
}
func (this *SecretSpec_CA) String() string {
	if this == nil {
		return "nil"
	}
	s := strings.Join([]string{`&SecretSpec_CA{`,
		`CA:` + strings.Replace(fmt.Sprintf("%v", this.CA), "CAInfo", "CAInfo", 1) + `,`,
		`}`,
	}, "")
	return s
}
func (this *SecretSpec_LeafCertificate) String() string {
	if this == nil {
		return "nil"
	}
	s := strings.Join([]string{`&SecretSpec_LeafCertificate{`,
		`LeafCertificate:` + strings.Replace(fmt.Sprintf("%v", this.LeafCertificate), "LeafCertInfo", "LeafCertInfo", 1) + `,`,
		`}`,
	}, "")
	return s
}
...
func (this *CAInfo) String() string {
	if this == nil {
		return "nil"
	}
	s := strings.Join([]string{`&CAInfo{`,
		...
		`CertBundle:` + fmt.Sprintf("%v", this.CertBundle) + `,`,
		`}`,
	}, "")
	return s
}
func (this *LeafCertInfo) String() string {
	if this == nil {
		return "nil"
	}
	s := strings.Join([]string{`&LeafCertInfo{`,
		`Certificate:` + fmt.Sprintf("%v", this.Certificate) + `,`,
		`}`,
	}, "")
	return s
...
func (m *SecretSpec) Unmarshal(dAtA []byte) error {
	...
	for iNdEx < l {
		...
		case 5:
			...
			m.SpecialSecretType = &SecretSpec_CA{v}
			iNdEx = postIndex
		case 6:
			if wireType != 2 {
			...
			m.SpecialSecretType = &SecretSpec_LeafCertificate{v}
			iNdEx = postIndex
		default:
			iNdEx = preIndex
			skippy, err := skipSpecs(dAtA[iNdEx:])
...
func (m *CAInfo) Unmarshal(dAtA []byte) error {
	l := len(dAtA)
	iNdEx := 0
	for iNdEx < l {
		preIndex := iNdEx
		...
		wireType := int(wire & 0x7)
		if wireType == 4 {
			return fmt.Errorf("proto: CAInfo: wiretype end group for non-group")
		}
		if fieldNum <= 0 {
			return fmt.Errorf("proto: CAInfo: illegal tag %d (wire type %d)", fieldNum, wire)
		}
		switch fieldNum {
		case 1:
			if wireType != 2 {
		...
		case 4:
			...
			m.CertBundle = append(m.CertBundle[:0], dAtA[iNdEx:postIndex]...)
			if m.CertBundle == nil {
				m.CertBundle = []byte{}
			}
			iNdEx = postIndex
...
func (m *LeafCertInfo) Unmarshal(dAtA []byte) error {
	l := len(dAtA)
	iNdEx := 0
	for iNdEx < l {
		preIndex := iNdEx
		...
		wireType := int(wire & 0x7)
		if wireType == 4 {
			return fmt.Errorf("proto: LeafCertInfo: wiretype end group for non-group")
		}
		if fieldNum <= 0 {
			return fmt.Errorf("proto: LeafCertInfo: illegal tag %d (wire type %d)", fieldNum, wire)
		}
		switch fieldNum {
		case 1:
			if wireType != 2 {
...
var fileDescriptorSpecs = []byte{
	...
	,
}
