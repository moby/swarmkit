...
// AuthorizeForwardRole checks for proper roles of caller. It can be manager who
// forward agent request or agent itself. It returns agent id.
func AuthorizeForwardRole(ctx context.Context, ou []string) (string, error) {
	// If the call is being done directly by an accepted role, return the CN
	cn, err := authorizeRole(ctx, ou)
	if err == nil {
		return cn, nil
	}

	// If the call is being done by a manager, return the forwarded CN
	_, err = authorizeRole(ctx, []string{ManagerRole})
	if err == nil {
