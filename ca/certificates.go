	...
	for i, cert := range parsedCerts {
		...
		if now.Before(cert.NotBefore) {
			return nil, nil, fmt.Errorf(
				...
				i+1, cert.Subject.CommonName, cert.NotBefore.UTC().Format(time.RFC1123), now.Format(time.RFC1123), x509.CertificateInvalidError{
					Cert:   cert,
					Reason: x509.Expired,
				})
		}
		if !allowExpired && now.After(cert.NotAfter) {
			return nil, nil, fmt.Errorf(
				...
				i+1, cert.Subject.CommonName, cert.NotAfter.UTC().Format(time.RFC1123), now.Format(time.RFC1123), x509.CertificateInvalidError{
					Cert:   cert,
					Reason: x509.Expired,
				})
		}
...
func GetRemoteCA(ctx context.Context, d digest.Digest, connBroker *connectionbroker.Broker) (RootCA, error) {
	...
	if d != "" {
		...
		if !verifier.Verified() {
			return RootCA{}, fmt.Errorf("remote CA does not match fingerprint. Expected: %s", d.Encoded())
		}
	}
