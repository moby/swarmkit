...
const (
	...
	PassphraseENVVar = "SWARM_PK_PASSWORD"
	...
	// KEK rotations.
	PassphraseENVVarPrev = "SWARM_PK_PASSWORD_PREV"
)
...
func (rca *RootCA) RequestAndSaveNewCertificates(ctx context.Context, paths CertPaths, role, secret string, picker *picker.Picker, transport credentials.TransportAuthenticator) (*tls.Certificate, error) {
	...
	signedCert, err := GetRemoteSignedCertificate(ctx, csr, role, secret, rca.Pool, picker, transport)
	if err != nil {
		return nil, err
	}
...
func NewRootCA(cert, key []byte) (RootCA, error) {
	...
	var (
		password, passwordPrev []byte
		priv                   crypto.Signer
	)

	// Attempt two distinct passwords, so we can do a hitless password rotation
	strPassword := os.Getenv(PassphraseENVVar)
	strPasswordPrev := os.Getenv(PassphraseENVVarPrev)
	...
	if err != nil {
		priv, err = helpers.ParsePrivateKeyPEMWithPassword(key, passwordPrev)
		if err != nil {
	...
	if strPassword != "" && !x509.IsEncryptedPEMBlock(keyBlock) {
		key, err = EncryptECPrivateKey(key, strPassword)
		if err != nil {
			return RootCA{}, err
		}
	}
...
func GetRemoteSignedCertificate(ctx context.Context, csr []byte, role, secret string, rootCAPool *x509.CertPool, picker *picker.Picker, creds credentials.TransportAuthenticator) ([]byte, error) {
	if rootCAPool == nil {
