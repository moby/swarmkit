...
func TestGetLocalRootCA(t *testing.T) {
	...
	_, err = ca.GetLocalRootCA(paths.RootCA)
	require.EqualError(t, err, "certificate key mismatch")
}
...
func TestGetRemoteSignedCertificateWithPending(t *testing.T) {
	...
	require.Greater(t, atomic.LoadInt64(&fakeCAServer.nodeStatusCalled), int64(1), "expected NodeCertificateStatus to have been polled more than once")
...
func TestNewRootCAInvalidCertAndKeys(t *testing.T) {
	...
	for i, invalid := range invalids {
		...
		require.Error(t, err, "expected error containing: \"%s\", test case (%d)", invalid.errorStr, i)
		require.Contains(t, err.Error(), invalid.errorStr, "%d", i)
	}
}
