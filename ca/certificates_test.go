...
func TestGetRemoteSignedCertificate(t *testing.T) {
	...
	response, err := ca.GetRemoteSignedCertificate(context.Background(), csr, tc.ManagerToken, tc.RootCA.Pool, tc.Remotes, nil, nil)
	assert.NoError(t, err)
	...
	response, err = ca.GetRemoteSignedCertificate(tc.Context, csr, tc.WorkerToken, tc.RootCA.Pool, tc.Remotes, nil, nil)
	assert.NoError(t, err)
	assert.NotNil(t, response.Certificate.Certificate)
	assert.NotNil(t, response.RootCABundle)
	parsedCerts, err = helpers.ParseCertificatesPEM(response.Certificate.Certificate)
...
func TestGetRemoteSignedCertificateNodeInfo(t *testing.T) {
	...
	info := make(chan api.IssueNodeCertificateResponse, 1)
	response, err := ca.GetRemoteSignedCertificate(context.Background(), csr, tc.WorkerToken, tc.RootCA.Pool, tc.Remotes, nil, info)
	assert.NoError(t, err)
	assert.NotEmpty(t, <-info)
	assert.NotNil(t, response.Certificate.Certificate)
	assert.NotNil(t, response.RootCABundle)
...
func TestGetRemoteSignedCertificateWithNilCABundle(t *testing.T) {
	...
	info := make(chan api.IssueNodeCertificateResponse, 1)
	response, err := ca.GetRemoteSignedCertificate(context.Background(), csr, tc.WorkerToken, tc.RootCA.Pool, remotes, nil, info)
	assert.NoError(t, err)
	assert.NotNil(t, response.Certificate.Certificate)
