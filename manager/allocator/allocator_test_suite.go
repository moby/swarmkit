...
func (suite *testSuite) TestAllocator() {
	...
	suite.Require().NoError(s.Update(func(tx store.Tx) error {
		...
		suite.Require().NoError(store.CreateNetwork(tx, n1))
		...
		suite.Require().NoError(store.CreateService(tx, s1))
		...
		suite.Require().NoError(store.CreateTask(tx, t1))
		...
		suite.Require().NoError(store.CreateTask(tx, t2))
		...
		suite.Require().NoError(store.CreateService(tx, sp1))
		...
		suite.Require().NoError(store.CreateTask(tx, tp1))
		...
		suite.Require().NoError(store.CreateService(tx, sp2))
		...
		suite.Require().NoError(store.CreateTask(tx, tp2))
	...
	suite.Require().NoError(s.Update(func(tx store.Tx) error {
		...
		suite.Require().NoError(store.CreateNetwork(tx, n2))
		return nil
	}))
	...
	suite.Require().NoError(s.Update(func(tx store.Tx) error {
		...
		suite.Require().NoError(store.CreateService(tx, s2))
		return nil
	}))
	...
	suite.Require().NoError(s.Update(func(tx store.Tx) error {
		...
		suite.Require().NoError(store.CreateTask(tx, t3))
		return nil
	}))
	...
	suite.Require().NoError(s.Update(func(tx store.Tx) error {
		suite.Require().NoError(store.CreateNetwork(tx, n3))
		return nil
	}))
	...
	suite.Require().NoError(s.Update(func(tx store.Tx) error {
		...
		suite.Require().NoError(store.CreateTask(tx, t5))
		return nil
	}))
	watchTask(suite.T(), s, taskWatch, false, isValidTask)

	suite.Require().NoError(s.Update(func(tx store.Tx) error {
		suite.Require().NoError(store.DeleteNetwork(tx, "testID3"))
		return nil
	}))
	watchNetwork(suite.T(), netWatch, false, isValidNetwork)

	suite.Require().NoError(s.Update(func(tx store.Tx) error {
		suite.Require().NoError(store.DeleteService(tx, "testServiceID2"))
		return nil
	}))
	watchService(suite.T(), serviceWatch, false, nil)
	...
	suite.Require().NoError(s.Update(func(tx store.Tx) error {
		...
		suite.Require().NoError(store.CreateTask(tx, t4))
		return nil
	}))
	watchTask(suite.T(), s, taskWatch, false, isValidTask)
	...
	suite.Require().NoError(s.Update(func(tx store.Tx) error {
		...
		suite.Require().NoError(store.UpdateNetwork(tx, n2))
		return nil
	}))
	watchNetwork(suite.T(), netWatch, false, isValidNetwork)
	...
	suite.Require().NoError(s.Update(func(tx store.Tx) error {
		...
		suite.Require().NoError(store.UpdateService(tx, s))
		return nil
	}))
	watchService(suite.T(), serviceWatch, false, nil)
	...
	suite.Require().NoError(s.Update(func(tx store.Tx) error {
		...
		suite.Require().NoError(store.UpdateTask(tx, t2))
		return nil
	}))
	watchTask(suite.T(), s, taskWatch, false, isValidTask)
	...
	suite.Require().NoError(s.Update(func(tx store.Tx) error {
		suite.Require().NoError(store.CreateNetwork(tx, n4))
		return nil
	}))
	watchNetwork(suite.T(), netWatch, false, isValidNetwork)

	suite.Require().NoError(s.Update(func(tx store.Tx) error {
		suite.Require().NoError(store.CreateNetwork(tx, n5))
		return nil
	}))
	watchNetwork(suite.T(), netWatch, true, nil)
	...
	suite.Require().NoError(s.Update(func(tx store.Tx) error {
		...
		suite.Require().NoError(store.CreateTask(tx, t6))
		return nil
	}))
	watchTask(suite.T(), s, taskWatch, true, nil)
	...
	suite.Require().NoError(s.Update(func(tx store.Tx) error {
		suite.Require().NoError(store.DeleteNetwork(tx, n4.ID))
		return nil
	}))
	watchNetwork(suite.T(), netWatch, false, isValidNetwork)
	watchTask(suite.T(), s, taskWatch, false, isValidTask)
	...
	suite.Require().NoError(s.Update(func(tx store.Tx) error {
		suite.Require().NoError(store.CreateService(tx, s3))
		return nil
	}))
	watchService(suite.T(), serviceWatch, false, nil)
	suite.Require().NoError(s.Update(func(tx store.Tx) error {
		suite.Require().NoError(store.CreateService(tx, s4))
		return nil
	}))
	...
	suite.Require().NoError(s.Update(func(tx store.Tx) error {
		...
		suite.Require().NoError(store.CreateTask(tx, t7))
		return nil
	}))
	watchTask(suite.T(), s, taskWatch, true, nil)
	...
	suite.Require().NoError(s.Update(func(tx store.Tx) error {
		suite.Require().NoError(store.DeleteService(tx, s3.ID))
		return nil
	}))
	watchService(suite.T(), serviceWatch, false, nil)
	watchTask(suite.T(), s, taskWatch, false, isValidTask)
...
func (suite *testSuite) TestAllocatorRestoreForDuplicateIPs() {
	...
	suite.Require().NoError(s.Update(func(tx store.Tx) error {
		...
		for i := 0; i != numsvcstsks; i++ {
			...
			suite.Require().NoError(store.CreateService(tx, svc))
		}
		return nil
	}))
	...
	for i := 0; i != numsvcstsks; i++ {
		suite.Require().NoError(s.Update(func(tx store.Tx) error {
			...
			suite.Require().NoError(store.CreateTask(tx, tsk))
			return nil
		}))
	}
...
func (suite *testSuite) TestNodeAllocator() {
	...
	suite.Require().NoError(s.Update(func(tx store.Tx) error {
		...
		suite.Require().NoError(store.CreateNetwork(tx, nUnused))
	...
	suite.Require().NoError(s.Update(func(tx store.Tx) error {
		suite.Require().NoError(store.CreateNode(tx, node2))
		return nil
	}))
	...
	suite.Require().NoError(s.Update(func(tx store.Tx) error {
		suite.Require().NoError(store.DeleteTask(tx, "task1"))
		return nil
	}))
	...
	// Remove a node and validate remaining node has 2 LB IP addresses
	suite.Require().NoError(s.Update(func(tx store.Tx) error {
		suite.Require().NoError(store.DeleteNode(tx, node2.ID))
		return nil
	}))
...
func (suite *testSuite) TestAllocateServiceConflictingUserDefinedPorts() {
	...
	suite.Require().NoError(s.Update(func(tx store.Tx) error {
		...
		if suite.NotNil(s1) {
			...
			suite.Require().NoError(store.UpdateService(tx, s1))
		}
		return nil
	}))
...
func (suite *testSuite) TestDeallocateServiceAllocate() {
	...
	suite.Require().NoError(s.Update(func(tx store.Tx) error {
		...
		suite.Require().NoError(store.CreateNetwork(tx, in))
		suite.Require().NoError(store.CreateService(tx, newSvc("testID1")))
		return nil
	}))
	...
	suite.Require().NoError(s.Update(func(tx store.Tx) error {
		suite.Require().NoError(store.DeleteService(tx, "testID1"))
		suite.Require().NoError(store.CreateService(tx, newSvc("testID2")))
		return nil
	}))
