...
func TestValidateContainerSpec(t *testing.T) {
	...
	bad2 := api.TaskSpec{
		Runtime: &api.TaskSpec_Container{
			Container: &api.ContainerSpec{
				Image: "image",
				Hosts: []string{"node1"}, // each element of hosts should be format of host:ip
			},
		},
	}
	...
	bad3 := api.TaskSpec{
		Runtime: &api.TaskSpec_Container{
			Container: &api.ContainerSpec{
				Image: "image",
				Hosts: []string{"node1:1.1.1.1", "node2:1.2.3.500"}, // ipv4: each element of hosts should be format of host:ip
			},
		},
	}
	...
	bad4 := api.TaskSpec{
		Runtime: &api.TaskSpec_Container{
			Container: &api.ContainerSpec{
				Image: "image",
				Hosts: []string{"node1:1fff:0:a88:85a3:ac1f"}, // ipv6: each element of hosts should be format of host:ip
			},
		},
	}
	...
	good1 := api.TaskSpec{
		Runtime: &api.TaskSpec_Container{
			Container: &api.ContainerSpec{
				Image: "image",
				Hosts: []string{"node1:1.1.1.1", "node2:1.2.3.120", "node1:ABCD:EF01:2345:6789:ABCD:EF01:2345:6789"},
				Mounts: []api.Mount{
					{
						Type:   api.Mount_MountType(0),
						Source: "/data",
