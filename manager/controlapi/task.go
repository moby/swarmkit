...
func (s *Server) ListTasksStream(request *api.ListTasksRequest, stream api.Control_ListTasksStreamServer) error {
	...
	for len(tasks) > maxTasksInResponse {
		...
		if err := stream.Send(&api.ListTasksResponse{Tasks: theseTasks}); err != nil {
			// chances are if there is an error it's because the client has
			// dropped off, but we can handle it this way anyway. not really a
			// better way to do so
			return err
		}
	}
	...
	return stream.Send(&api.ListTasksResponse{Tasks: tasks})
}
