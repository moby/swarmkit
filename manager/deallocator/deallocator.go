...
// Deallocator is used when users request services or service-level resources
// to be deleted asynchronously, ie only when their tasks have fully shut down.
// Note that this behaviour is opt-in only, and not that by default services
// and their resources are deleted right away.
//
// It’s worth noting that this component’s role is quite different from
