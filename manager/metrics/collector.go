...
func (c *Collector) handleNodeEvent(event events.Event) {
	...
	if prevNode != nil && newNode != nil && prevNode.GetStatus().GetState() == newNode.GetStatus().GetState() {
		return
	}

	if prevNode != nil {
		nodesMetric.WithValues(strings.ToLower(prevNode.GetStatus().GetState().String())).Dec(1)
	}
	if newNode != nil {
		nodesMetric.WithValues(strings.ToLower(newNode.GetStatus().GetState().String())).Inc(1)
	}
}
...
func (c *Collector) handleTaskEvent(event events.Event) {
	...
	// Skip updates if nothing changed.
	if prevTask != nil && newTask != nil && prevTask.GetStatus().GetState() == newTask.GetStatus().GetState() {
		return
	}

	if prevTask != nil {
		tasksMetric.WithValues(
			strings.ToLower(prevTask.GetStatus().GetState().String()),
		).Dec(1)
	}
	if newTask != nil {
		tasksMetric.WithValues(
			strings.ToLower(newTask.GetStatus().GetState().String()),
		).Inc(1)
	}
}
