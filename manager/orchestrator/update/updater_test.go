...
func TestUpdaterPlacement(t *testing.T) {
	...
	for _, slot := range updatedTasks {
		for _, task := range slot {
			assert.Equal(t, task.Spec.Placement.Constraints[0], "node==*")
			assert.Equal(t, task.Status.State, api.TaskStateRunning)
	...
	assert.Equal(t, updatedTaskCount, originalTaskCount)
}
