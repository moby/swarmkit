...
func TestGCWAL(t *testing.T) {
	...
	for i := 0; i != proposals; i++ {
		_, err := proposeHugeValue(t, nodes[1], fmt.Sprintf("id%d", i))
		assert.NoError(t, err, "failed to propose value")
	}
	...
	_, err := raftutils.ProposeValue(t, raftutils.Leader(nodes), "newnode")
	assert.NoError(t, err, "failed to propose value")
...
func proposeHugeValue(t *testing.T, raftNode *raftutils.TestNode, nodeID ...string) (*api.Node, error) {
	nodeIDStr := "id1"
	if len(nodeID) != 0 {
		nodeIDStr = nodeID[0]
	}
