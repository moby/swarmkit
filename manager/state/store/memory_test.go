...
func TestStoreNode(t *testing.T) {
	...
	s.View(func(readTx ReadTx) {
		...
		foundNodes, err = FindNodes(readTx, ByName("name1", "name2"))
		assert.NoError(t, err)
...
func TestStoreService(t *testing.T) {
	...
	s.View(func(readTx ReadTx) {
		...
		foundServices, err = FindServices(readTx, ByName("name1", "name2"))
		assert.NoError(t, err)
		assert.Len(t, foundServices, 2)
		foundServices, err = FindServices(readTx, ByName("name1", "name2", "name4"))
		assert.NoError(t, err)
		assert.Len(t, foundServices, 2)
