...
func (mp *MockProposer) ChangesBetween(from, to *api.Version) ([]state.Change, error) {
	...
	lastIndex := mp.changes[len(mp.changes)-1].Version.GetIndex()

	if to.GetIndex() > lastIndex || from.GetIndex() > lastIndex {
		return nil, errors.New("out of bounds")
	}

	for _, change := range mp.changes {
		if change.Version.GetIndex() > from.GetIndex() && change.Version.GetIndex() <= to.GetIndex() {
