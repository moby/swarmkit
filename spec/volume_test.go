	...
	set := []*conv{
		...
		},
		{
			...
			to: &api.VolumeSpec{
				...
				DriverConfig: &api.Driver{Name: "bar", Options: map[string]string{}},
			},
		},
		{
			...
			to: &api.VolumeSpec{
				Annotations:  api.Annotations{Name: "foo", Labels: make(map[string]string)},
				DriverConfig: &api.Driver{Name: "bar", Options: map[string]string{"o1": "v1"}},
			},
		},
	}
	...
	set := []*conv{
		{
			from: &api.VolumeSpec{
				Annotations:  api.Annotations{Name: "foo", Labels: make(map[string]string)},
				DriverConfig: &api.Driver{Name: "bar"},
			},
