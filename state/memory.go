...
func (tx tx) commitChangelist() error {
	for _, c := range tx.changelist {
		...
		case EventCreateNode:
			tx.s.nodes[v.Node.Spec.ID] = v.Node
		case EventUpdateNode:
			tx.s.nodes[v.Node.Spec.ID] = v.Node
		case EventDeleteNode:
			delete(tx.s.nodes, v.Node.Spec.ID)
		}
...
func (nodes nodes) Create(n *api.Node) error {
	if _, ok := nodes.deleted[n.Spec.ID]; !ok {
		if _, ok := nodes.s.nodes[n.Spec.ID]; ok {
			return ErrExist
		}
		if _, ok := nodes.updated[n.Spec.ID]; ok {
			return ErrExist
		}
	} else {
		delete(nodes.deleted, n.Spec.ID)
	}

	nodes.updated[n.Spec.ID] = n
...
func (s *MemoryStore) listNodes(deleted map[string]struct{}, updated map[string]*api.Node) []*api.Node {
	...
	for _, n := range s.nodes {
		if deleted != nil {
			if _, ok := deleted[n.Spec.ID]; ok {
				continue
			}
		}
		if updated != nil {
			if _, ok := updated[n.Spec.ID]; ok {
				continue
			}
		}
